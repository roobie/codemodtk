...
export async function upgradeDeps(
	...
	await Promise.all(
		Object.keys(imports)
			...
			.map(async (pkg) => {
				...
				if (!semver.valid(currentVersion) && !process.argv.includes("force")) {
					logs &&
						logger(
							colors.yellow(
				...
				if (currentVersion !== latestVersion) {
					logs &&
						logger(`upgrading ${pkg} ${currentVersion} -> ${latestVersion}.`);
	...
	for (const [pkg, minVer] of Object.entries(requiredMinVersion)) {
		if (imports[pkg]) {
			...
			if (
				!currentVersion ||
				semver.lt(semver.coerce(currentVersion)!, semver.coerce(minVer)!)
			) {
				logs && logger(`upgrading ${pkg} ${currentVersion} -> ${minVer}.`);

				upgradeFound = true;
				imports[pkg] = url?.at(minVer).url ?? imports[pkg];
			}
		}
	}
...
export async function update(cwd: string = process.cwd()) {
